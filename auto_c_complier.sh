...
B_Blue='\033[0;94m'
B_Purple='\033[0;95m'
B_Cyan='\033[0;96m' B_White='\033[0;97m'
Bold='\033[1m'
Italic='\033[3m'
Reset='\033[0m'
...
function read_makefile_flags() {
    local cc_line=$(grep '^CC\s*=' Makefile)
    if [ -n "$cc_line" ]; then
        CC=$(echo "$cc_line" | sed 's/CC\s*=\s*//')
    else
        echo "Error: CC not defined in Makefile!" >&2
        exit 1
    fi

    local cflags_line=$(grep '^CFLAGS\s*=' Makefile)
    if [ -n "$cflags_line" ]; then
        CFLAGS=$(echo "$cflags_line" | sed 's/CFLAGS\s*=\s*//')
    fi
}
...
fswatch -o --exclude 'Makefile' $(find . -name '*.c' -o -name '*.h') | while read events; do
    compile_file
done &
